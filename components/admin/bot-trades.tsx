...
import { deleteBotTradeAction } from "@/app/actions/bots";
...
export function BotTrades({ trades }: BotTradesProps) {
  ...
  function handleDelete() {
    ...
    startTransition(async () => {
      const r = await deleteBotTradeAction(deleteId);
  ...
  return (
    <>
              ...
              <div key={trade.id} className="p-4 hover:bg-muted/10 transition-colors">
                <div className="flex items-start justify-between gap-3 mb-2 flex-wrap">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="font-mono text-xs">{trade.coins?.symbol}</span>
                  ...
                  <div className="flex items-center gap-1">
                    {isOpen && (
                      <Button size="sm" variant="outline" onClick={() => setCloseId(trade.id)} disabled={isPending}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs font-mono mt-2">
                  <Field label="Monto" value={`${formatUSDT(Number(trade.amount))} USDT`} />
                  <Field
                    label="Entrada"
                  ...
                  <Field
                    ...
                    value={new Date(isOpen ? trade.opened_at : trade.closed_at).toLocaleString("es-AR", {
                      day: "2-digit",
                      month: "2-digit",
                      hour: "2-digit",
                      minute: "2-digit",
      ...
      <Dialog open={!!closeId} onOpenChange={(o) => !o && setCloseId(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>¿Cerrar operación del bot?</DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={!!deleteId} onOpenChange={(o) => !o && setDeleteId(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>¿Borrar trade del historial?</DialogTitle>
            <DialogDescription>
              {trades.find((t) => t.id === deleteId)?.status === "open"
                ? "Si está abierto, se libera el saldo lockeado."
                : "Solo borra el registro."}
            </DialogDescription>
          </DialogHeader>
          {error && <p className="text-sm text-destructive">{error}</p>}
          <DialogFooter>
